...
def parse_args():
    ...
    parser.add_argument(
        "--test_file", type=str, default='ntuadl2023hw1/test.json', help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--max_length",
        type=int,
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        extension = "json"
        # ref: https://huggingface.co/docs/datasets/loading#json
    ...
    total_context = json.loads(context_dir.read_text())
    ...
    def preprocess_function(examples):
        ...
        second_sentences = [
            [f"{total_context[idx]}" for idx in selections] for i, selections in enumerate(examples[question_header_name])  # i: 第 i 個 data， 每個 data 有4個 paragraph selection 
        ]
        ...
        # for test data
        tokenized_inputs['label'] = [0 for i, selections in enumerate(examples[question_header_name])]

        return tokenized_inputs
    raw_test_data = raw_datasets['test']
    ...
    # Test!
    # output be designed as the file after preprocess_qa.py
    # thus the output here do not need preprocess
    # can be used in stage2_test.py directly
    ...
    for i, pred in enumerate(predictions_idx):
        context_id = raw_test_data["paragraphs"][i][pred]
        data = {
            "id": raw_test_data["id"][i],
            "question": raw_test_data["question"][i],
            "context": total_context[context_id]
        }
        output_json["data"].append(data)
    
    json.dump(output_json, open(args.output_json, 'w'), indent=2, ensure_ascii=False)
