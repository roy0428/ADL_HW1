...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='ntuadl2023hw1/valid_qa.json', 
    ...
    parser.add_argument(
        "--num_warmup_steps", type=int, default=300, help="Number of steps for the warmup in the lr scheduler."
    )
    parser.add_argument("--output_dir", type=str, 
                        default='result_2', help="Where to store the final model.")
    parser.add_argument("--seed", type=int, default=7414, help="A seed for reproducible training.")
    parser.add_argument(
        "--doc_stride",
        type=int,
        default=128,
...
def main():
    ...
    def prepare_train_features(examples):
        # with open(args.context_file, 'r') as json_file:
        ...
        # contents = [data[context] for context in examples[context_column_name]]
        # examples['context'] = contents
        # ipdb.set_trace()
    ...
    # with open(args.context_file, 'r') as json_file:
    #         data = json.load(json_file)
    # contents = [data[context] for context in train_dataset[context_column_name]]
    # ipdb.set_trace()
    ...
    def prepare_validation_features(examples):
        # with open('ntuadl2023hw1/context.json', 'r') as json_file:
        #     data = json.load(json_file)
        # Some of the questions have lots of whitespace on the left, which is not useful and will make the
        # truncation of the context fail (the tokenized question will take a lots of space). So we remove that
        # left whitespace
    ...
    # with open(args.context_file, 'r') as json_file:
    #     data = json.load(json_file)
    # eval_contents = [data[context] for context in eval_examples[context_column_name]]
    # ipdb.set_trace()
    ...
    if args.do_predict:
        ...
        # extension = args.test_file.split(".")[-1]
        # raw_datasets = load_dataset(extension, data_files=test_data_files)
        predict_examples = raw_datasets['test']
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if completed_steps % 346 == 0:
                ...
                data = {
                    ...
                    "exact_match_metric": eval_metric["exact"]
                }
                curve.append(data)
    ...
    if args.output_dir is not None:
        ...
        json.dump({'curve': curve}, open(os.path.join(args.output_dir, "training_curve.json"), 'w'), indent=2)
        if accelerator.is_main_process:
            tokenizer.save_pretrained(args.output_dir)
            if args.push_to_hub:
